#!/usr/bin/env node
/**
 * Run all D1 remote migrations in dependency order.
 * Skips migrations that are already applied (duplicate column/table).
...
function run(name, file) {
  const result = spawnSync(
    'npx',
    ['wrangler', 'd1', 'execute', 'clrhoa_db', '--remote', '--file', path.join(root, 'scripts', file)],
...
console.log('Running all remote D1 migrations in order...\n');
...
run('db:arb-audit-ip', 'schema-arb-audit-ip.sql');
...
runScript('db:vendors-website', ['run-vendors-website-migration.js', '--remote']);
...
run('db:directory-logs-ip', 'schema-directory-logs-ip.sql');
...
run('db:phase6-approved', 'schema-phase6-preapproval-approved.sql');
...
run('db:backup-config-last-run', 'schema-backup-config-last-run.sql');
run('db:backup-config-google-last', 'schema-backup-config-google-last.sql');
...
run('db:owners-updated-by', 'schema-owners-updated-by.sql');
run('db:owners-lot-number', 'schema-owners-lot-number.sql');
run('db:site-feedback', 'schema-site-feedback.sql');
run('db:sms-feature-requests', 'schema-sms-feature-requests.sql');
run('db:page-views', 'schema-page-views.sql');
run('db:news-items', 'schema-news-items.sql');
run('db:news-items-placement', 'schema-news-items-placement-and-images.sql');
runScript('db:meetings-post-to-public', ['run-meetings-post-to-public.js', '--remote']);
runScript('db:vendors-show-on-public', ['run-vendors-show-on-public.js', '--remote']);
run('db:vendor-audit', 'schema-vendor-audit.sql');
