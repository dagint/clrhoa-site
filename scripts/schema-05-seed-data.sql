...
-- ============================================================================
-- This file contains initial data inserts for:
-- - Compliance requirements (15 Florida HOA Statute 720.303(4) requirements)
-- - Public documents (placeholder rows for bylaws, covenants, forms)
-- - Backup configuration (default settings)
--
-- Usage:
--   npm run wrangler d1 execute clrhoa_db --local --file=./scripts/consolidated/schema-05-seed-data.sql
--   npm run wrangler d1 execute clrhoa_db --remote --file=./scripts/consolidated/schema-05-seed-data.sql
--
-- IMPORTANT: Run after all schema files (01-04) have been executed
-- ============================================================================

-- ----------------------------------------------------------------------------
-- Florida HOA Compliance Requirements (FL Statute 720.303(4))
-- Based on effective date: January 1, 2025
-- ----------------------------------------------------------------------------

INSERT OR IGNORE INTO compliance_requirements (id, statute_ref, title, description, category, posting_location, posting_deadline_days, retention_years, requires_annual_update, is_repeating, sort_order) VALUES
...
-- ----------------------------------------------------------------------------
-- Public Documents (Placeholder Rows)
-- Until first upload, these serve from content collection (src/content/public-documents/)
-- After upload, file_key points to R2 storage
-- ----------------------------------------------------------------------------
...
INSERT OR IGNORE INTO backup_config (id, google_drive_enabled, schedule_type, schedule_hour_utc, include_r2_manifest, include_r2_files)
VALUES (1, 0, 'daily', 2, 0, 0);
